...
class MyDB:
    def __init__(self):
        self.db_con = lite.connect('database/main.db', isolation_level=None)
        if self.db_con == None:
            print("fail")
            return
    ...
    def insert_cash_sale(self,values):
        self.db_cur.execute("INSERT INTO sale('Company','Phone Number','Amount','Alt Number','Type','Status')\
                            VALUES(?, ?, ?, ?, ?, ?)", values)

    def insert_credit_sale(self,values):
        self.db_cur.execute("INSERT INTO sale('Company','Phone Number','Amount','Alt Number','Party Name','Type','Status')\
                            VALUES(?, ?, ?, ?, ?, ?, ?)", values)

    def insert_cash_in_payment(self,values):
        self.db_cur.execute("INSERT INTO payment('Party Name', 'Alt Number', 'Amount') VALUES(?, ?, ?)", values)

    def insert_cash_out_payment(self,values):
        self.db_cur.execute("INSERT INTO payment('Party Name', 'Amount', 'Particular') VALUES(?, ?, ?)", values)
